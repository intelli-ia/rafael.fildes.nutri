...
export interface SinglePricingCardProps {
  ...
  featuresBadge?: { icon: LucideIcon; text: string }
  primaryButton: {
    text: string
    icon: LucideIcon
    href?: string
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="p-6 md:p-8 md:w-1/2 flex flex-col">
          ...
          <div className="flex items-baseline mb-6 gap-2">
            ...
            {price.original && (
              <span className="text-muted-foreground line-through">{price.original}</span>
            )}
            {price.discount && (
              <Badge variant="outline" className={`border-green-400/30 text-green-400 ${price.discountBadgeClassName || ""}`}>
                {price.discount}
              </Badge>
            )}
          </div>
          ...
          <div className="mt-auto space-y-3">
            <Button className="w-full gap-2 group/btn" size="lg" onClick={primaryButton.onClick} asChild={!!primaryButton.href}>
              {primaryButton.href ? (
                <Link href={primaryButton.href}>
                  <PrimaryButtonIcon className="h-4 w-4" />
                  <span>{primaryButton.text}</span>
                  {ChevronIcon && <ChevronIcon className="h-4 w-4 ml-auto transition-transform group-hover/btn:translate-x-1" />}
                </Link>
              ) : (
                <>
                  <PrimaryButtonIcon className="h-4 w-4" />
            ...
            {secondaryButton && SecondaryButtonIcon && (
              <Button variant="outline" className="w-full gap-2" size="lg" onClick={secondaryButton.onClick} asChild={!!secondaryButton.href}>
        ...
        <div className="p-6 md:p-8 md:w-1/2 md:border-l border-border/50">
          <h4 className="font-semibold mb-4">{featuresTitle}</h4>
